    ...
    return (
        <div className="bg-gray-50 py-8">
            ...
            <div className="flex justify-center">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 w-full px-4 sm:px-8 lg:px-16">
                    {Categories?.slice(1).map((Category: CategoryType, index: number) => (
                        <Link key={index} href={`/Category/${Category.name}`}>
                            <div className="group relative bg-white border border-gray-200 p-4 rounded-2xl shadow-lg transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:bg-gray-100 cursor-pointer">
                                <img
                                    src={Category.image}
                                    alt={Category.name}
