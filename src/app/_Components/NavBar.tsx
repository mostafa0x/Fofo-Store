...
export default function NavBar() {
    ...
    function SetCategory(NameCategory: string) {
        ...
        <div className="navbar fixed top-0 bg-base-100 shadow-sm z-[60]">
            ...
            <div className="flex gap-2">
                {UserToken ? isLoadingCartIcon ? <div role="status">
                    ...
                    <div ref={CartList} tabIndex={0} role="button" className="btn btn-ghost btn-circle pr-4">
                        <div className="indicator items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="flex h-15 w-8 text-center items-center" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" /> </svg>
                            <span className="badge badge-sm indicator-item">{MyCart?.MyCart?.length ?? 0}</span>
                        </div>
                    </div>
                    <div
                        tabIndex={0}
                        ...
                        <div className="card-body">
                            ...
                            <span className="text-info">Total Price: {MyCart?.Totalprice ?? 0} EGP</span>
                            <div className="card-actions">
                                <Link onClick={() => List()} href={"/Cart"}><button className="btn btn-primary btn-block">View cart</button></Link>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {LoadingIconOptions ? <div role="status">
                    <svg aria-hidden="true" className="inline w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-gray-600 dark:fill-gray-300" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                    </svg>
                    ...
                    <select ref={Selector} defaultValue="All" className="select " onClick={(e) => {
                ...
                {isUserLoading ? <div role="status">
                    ...
                    {UserToken ?
                        <ul
                            tabIndex={0}
                            className="menu menu-sm dropdown-content text-3xl font-semibold bg-gray-300 rounded-box z-1 mt-3 w-52 p-2 shadow">
                            <li className=' rounded-xl hover:bg-slate-600' onClick={() => LogOut()}><i>Logout</i></li>
                        </ul>
                        : null}
                </div>}
