...
export default function NavBar() {
    ...
    return (
        <div className="navbar bg-white fixed top-0 z-[55] px-5 py-4 w-full">
            <div className="flex items-center justify-between w-full">
                <div className='flex gap-6'>
                    <Link href={"/"}>
                        <i className="btn btn-ghost text-xl">Fofo Store</i>
                    </Link>
                    <input
                        onKeyDown={(e) => { if (e.key == "Enter") { Search(SearchBar?.current?.value ?? "") } }}
                        ref={SearchBar}
                        type="text"
                        placeholder="Search here for products"
                        className="outline-none max-w-full w-3/4 sm:w-1/2 lg:w-1/3"
                        style={{ width: `100%` }}
                    />
                    {LoadingIconOptions ? (
                        <div role="status">
                        ...
                        <div className='flex gap-4'>
                            <select
                                id='mySelect'
                                onClick={() => selectMax(Op?.current?.value?.length ?? 0, (Op.current?.value ?? "All"))}
                                ref={Op}
                                className='outline-none w-1/3 sm:w-1/4'
                                style={{ width: `100%` }}
                            >
                            ...
                            <button
                                ...
                                type="button"
                                className="font-semibold text-sm text-white bg-gray-800 hover:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-300 rounded-lg px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-gray-700 dark:border-gray-700"
                            >
                                Search
                            </button>
                        </div>
                ...
                {isUserLoading ? (
                    ...
                    <>
                        {UserToken ? (
                            ...
                            <Link href={"/Login"}>
                                <i className="fa-regular fa-user text-5xl btn btn-ghost"></i>
                            </Link>
                        )}
                    </>
                )}
